		...
		render : function(nst) {
			...
			function loadScripts() {
				...
				if (s.theme && typeof s.theme != "function" && s.theme.charAt(0) != '-' && !ThemeManager.urls[s.theme])
					ThemeManager.load(s.theme, 'themes/' + s.theme + '/editor_template_src.js');
		...
		init : function() {
			...
			if (s.theme) {
				if (typeof s.theme != "function") {
					...
					if (t.theme.init)
						t.theme.init(t, ThemeManager.urls[s.theme] || tinymce.documentBaseURL.replace(/\/$/, ''), '{{ imagechooser_path }}');
				} else {
					t.theme = s.theme;
				}
			}
